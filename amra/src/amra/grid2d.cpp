...
void Grid2D::CreateSearch() //根据 GRID 的值来选择合适的启发式函数添加到 m_heurs 容器中，
{
	if (GRID == 4) {		//如果是四联通
		m_heurs.emplace_back(new ManhattanDist(this));
		//一是创建 ManhattanDist 类的一个新对象，二是将该对象的指针添加到 m_heurs 容器中
		//如果 GRID 的值为 4（四联通网格），则创建一个 ManhattanDist（曼哈顿距离）启发式函数对象，并将其指针添加到 m_heurs 容器中
	}
	else if (GRID == 8) {		//如果是八联通
	...
	m_search = std::make_unique<AMRAStar>(  //<AMRAStar>模板函数
...
{
	assert(!m_goal_set);
	//确保起始点只能设置一次，如果重复调用会触发断言错误（程序终止）。
	m_goal_id = getOrCreateState(d1, d2);
	m_goal_set = true;
	//将成员变量 m_start_set 设为 true，表示起始点已初始化。

	m_g1 = d1;
	m_g2 = d2;
	//将传入的坐标备份到类的成员变量 m_s1 和 m_s2，可能用于后续路径规划或其他操作。
	printf("Goal: {%d, %d},\n", m_g1, m_g2);
}
...
{
	int d1s, d2s, d1g, d2g;
	if (!m_start_set)  //确保起点未设置
	{
		// set random start
		m_map->GetRandomState(d1s, d2s);//生成随机坐标
	...
	m_search->set_start(m_start_id);//调用搜索器的方法，将路径起点信息传递给搜索器
	m_search->set_goal(m_goal_id);//终点同起点
	...
	std::vector<int> solution;//路径序列
	std::vector<int> action_ids; //对应动作
    int solcost;
	...
	if (result && save)
	{
		std::vector<MapState> solpath;//MapState 是一个 自定义类型（可能是类或结构体）
		//定义一个动态数组 solpath，用于存储一系列 MapState 对象，通常表示 路径规划的结果路径。
		convertPath(solution, solpath);//将抽象路径转换成
		m_map->SavePath(solpath);//将生成的路径放入/dat/solutions/文件夹下

		double initial_t, final_t;//初始解、最终解时间
		int initial_c, final_c, total_e;//初始成本解、最终成本解、总扩展节点数
		...
		if (!exists)
		{
			STATS << "TotalExpansions," //总扩展节点数
					<< "InitialSolutionTime,FinalSolutionTime," //初始解耗时（秒），最终解总耗时（秒）
